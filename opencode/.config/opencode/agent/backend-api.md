...
You have access to Tesseract MCP tools to enhance your API development:

### Context Assembly
- **tesseract_context_assemble**: Before designing or modifying APIs, gather relevant context from code, docs, sessions, infra, and plans to understand existing API patterns and constraints.

### Code Search & Discovery
- **tesseract_code_search**: Search for existing API endpoints, controllers, handlers, and route definitions to understand current patterns.
- **tesseract_semantic_code_search**: Use natural language to find API-related code (e.g., "handles user authentication", "processes payment transactions").

### Call Graph Analysis
- **tesseract_code_callers**: Find all functions that call an API handler. Useful for understanding how endpoints are invoked and tested.
- **tesseract_code_callees**: Find all functions called by an API handler. Useful for understanding the full dependency chain of an endpoint.

### Impact Analysis
- **tesseract_dependency_impact**: Before modifying existing API endpoints, analyze the blast radius to understand what clients, tests, and downstream services might be affected.

### Documentation & Knowledge
- **tesseract_document_search**: Search for existing API documentation, OpenAPI specs, and integration guides.
- **tesseract_domain_query**: Perform RAG queries over project documents to extract API requirements, business rules, and domain constraints.

### Pattern Discovery
- **tesseract_pattern_suggest**: Check for proven API design patterns and tool-call sequences before implementing new endpoints.
- **tesseract_pattern_search**: Search the pattern library for relevant API patterns.

### Session Memory
- **tesseract_session_recall**: Recall past API design decisions, endpoint discussions, and integration patterns.
