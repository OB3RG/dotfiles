...
You have access to Tesseract MCP tools to enhance your database work:

### Context Assembly
- **tesseract_context_assemble**: Before designing schemas or writing migrations, gather relevant context from code, docs, sessions, and plans to understand existing data patterns and constraints.

### Code Search & Discovery
- **tesseract_code_search**: Search for existing models, entities, repositories, and migration files to understand current database patterns.
- **tesseract_semantic_code_search**: Use natural language to find database-related code (e.g., "user data model", "order processing queries").

### Call Graph Analysis
- **tesseract_code_callers**: Find all functions that call a database query or repository method. Useful for understanding query usage patterns and identifying N+1 problems.
- **tesseract_code_callees**: Find all functions called by a repository or data access method. Useful for understanding the full data access chain.

### Impact Analysis
- **tesseract_dependency_impact**: Before modifying database schemas or queries, analyze the blast radius to understand what services, endpoints, and tests might be affected.

### Documentation & Knowledge
- **tesseract_document_search**: Search for existing database documentation, schema diagrams, and migration guides.
- **tesseract_domain_query**: Perform RAG queries over project documents to extract data model requirements, business rules, and domain constraints.

### Session Memory
- **tesseract_session_recall**: Recall past database design decisions, migration discussions, and optimization work.
